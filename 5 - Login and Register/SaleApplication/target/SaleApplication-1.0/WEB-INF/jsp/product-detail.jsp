    ...
    <c:forEach var="comment" items="${product.commentCollection}"  >
        <div class="row">
            <div class="col-md-2" style="padding: 10px">
                <c:choose>
                    <c:when test="${comment.userId.avatar != null && comment.userId.avatar.startsWith('https') == true}">
                        <img style="width: 80px; height: 80px" class="rounded-circle" src="${comment.userId.avatar}" alt="${product.name}" />
                    </c:when>
                    <c:when test="${comment.userId.avatar == null || comment.userId.avatar.startsWith('https') != true}">
                        <img  style="width: 80px; height: 80px" class="rounded-circle" src="<c:url value="/images/default.jpg" />" style="width: 100px"/>
                    </c:when>
                </c:choose>
            </div>
